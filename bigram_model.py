...
def main():
    ...
    # Create a Bigram model instance
    model = Bigram()
    train_file = "Data/SherlockHolmes-train.txt"

    # Experiment with different values for k
    k_values = [0.1, 0.5, 1.0, 1.5]
    best_perplexity = float('inf')
    best_k = None
    for k in k_values:
        model.train(train_file, k=k)

        # Generate 2 sentences
        sent1 = model.generate_sentence()
        sent2 = model.generate_sentence()

        # update best k if current perplexity is lower
        unigram_perplexity, bigram_perplexity = model.test(train_file)
